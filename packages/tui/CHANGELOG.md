...
- Stopped long flicker when moving or resizing an omp pane in Warp. Resize no longer borrows the alternate screen there (override with `PI_TUI_RESIZE_IN_PLACE=0`), and a height-only ±1 SIGWINCH during the post-resize cursor probe no longer restarts that borrow.
